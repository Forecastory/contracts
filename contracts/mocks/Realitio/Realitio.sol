...
contract Realitio is BalanceHolder {
    ...
    modifier stateOpen(bytes32 question_id) {
        ...
        require(
            opening_ts == 0 || opening_ts <= uint32(now),
    ...
    modifier bondMustDouble(bytes32 question_id) {
        ...
        require(
            msg.value >= (questions[question_id].bond.mul(2)),
    ...
    {
        questions[question_id].bounty = questions[question_id].bounty.add(
            msg.value
        );
        ...
        internal
    {
        ...
        commitments[commitment_id].reveal_ts = uint32(now).add(
            commitment_timeout
        );
    }
        ...
        if (answer == best_answer) {
            if (payee == NULL_ADDRESS) {
                ...
                _payPayee(
                    question_id,
                    payee,
                    queued_funds.sub(answer_takeover_fee)
                );
